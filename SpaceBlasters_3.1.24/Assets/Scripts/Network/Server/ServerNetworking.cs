...
public class ServerNetworking : IDisposable
{
    ...
    private NetworkObject soldierPrefab;
    ...
    public ServerNetworking(NetworkManager networkManager, NetworkObject soldierPrefab)
    {
        ...
        NetworkManager.ConnectionApprovalResponse response)
    {
        ...
        _= CreatePlayerWithLatency(request.ClientNetworkId);
        response.CreatePlayerObject = false;


    }

    public async  Task CreatePlayerWithLatency(ulong clientId)
    {
        await Task.Delay(500);
        NetworkObject soldier = GameObject.Instantiate(soldierPrefab, RespawnLocation.GenerateRandomPos(), Quaternion.identity);

        soldier.SpawnAsPlayerObject(clientId);
    }
